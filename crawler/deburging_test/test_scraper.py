'''
현재 코드는 웹 스크래핑을 위한 디버깅용 테스트 코드입니다.
현재 사용하고 있는 라이브러리는 BeautifulSoup입니다. 그리고 2로 plan B로 selenium을 사용할 예정입니다.
'''
...
def debug_scrape_raw_text(url):
    ...
    try:
        ...
        if response.status_code == 200 and len(response.text) > 100:
            ...
            return {"URL": url, "성공 여부": "성공 (Plan A)", "글자 수": text_len, "본문 미리보기": response.text[:200].replace('\n', ' ')}
        else:
    ...
    try:
        ...
        if text_len > 100:
            ...
            return {"URL": url, "성공 여부": "성공 (Plan B)", "글자 수": text_len, "본문 미리보기": cleaned_text[:200].replace('\n', ' ')}
        else:
            print("   🚫 [Plan B 실패] 웹사이트가 비어있거나 보안에 막혔습니다.")
            return {"URL": url, "성공 여부": "실패 (내용 없음)", "글자 수": 0, "본문 미리보기": ""}
            
    except Exception as e:
        print(f"   🚫 [Plan B 에러] 차단됨: {e}")
        return {"URL": url, "성공 여부": f"완전 실패 ({e})", "글자 수": 0, "본문 미리보기": ""}

# ==========================================
# 실행부: 10개의 URL 테스트 및 CSV(구글 시트용) 송출
# ==========================================
if __name__ == "__main__":
    import urllib3
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning) # SSL 경고 숨김
    ...
    print("=" * 60)
    print("🕵️‍♂️ [스크래핑 보안 돌파 테스트 시작] - 총 10개 URL")
    print("=" * 60)
    ...
    for url in test_urls:
        ...
        time.sleep(2) # 서버에 무리를 주지 않기 위한 매너 타임

    # 결과를 구글 시트에서 열 수 있는 CSV로 저장
    df = pd.DataFrame(results)
    csv_filename = "scraping_test_results.csv"
    df.to_csv(csv_filename, index=False, encoding='utf-8-sig')

    print("\n" + "=" * 60)
    print(f"🎯 [최종 성적] 10개 중 {success_count}개 성공 (성공률: {success_count/10*100}%)")
    print(f"💾 [파일 저장 완료] 결과가 '{csv_filename}' 파일로 송출되었습니다.")
    print("👉 구글 시트에서 [파일] -> [가져오기] -> [업로드]로 이 CSV 파일을 열어보세요!")
    print("=" * 60)